...
let paid_holidays = 14;
let national_holidays = 13; // China
...
let summer_vacation = 60;
let winter_vacation = 30;
...
let old_coolphone_last_years = 2;
let coolphone_cost = 999;
let first_coolphone_age = 21;
let total_number_of_coolphones = () => { return Math.ceil((max_age - first_coolphone_age) / old_coolphone_last_years) };
let total_coolphone_cost = () => { return toggles["toggle_coolphone"] == 1 ? total_number_of_coolphones() * coolphone_cost : 0 };
let age_bought_big_city_apartment = 40;
let cost_big_city = 3;
let size_big_city = 10;
...
let cost_small = 1.5;
let size_small = 30;
let total_cost_small = () => { return toggles["toggle_small_city"] == 1 ? cost_small * 100 * size_small * 100 : 0 }
...
let age_quit_job = 36;
let startup_hours_per_day = 10;
let startup_work_days_per_week = 6;
...
let lifelong_cost = {
  ...
  "coolphones": total_coolphone_cost,
...
let lines = [
  ...
  `You started working when you were ${clickable( "work_start_age")}, retired at the age of ${clickable( "work_end_age")}; that's ${dependent( "work_years")} working years. Retirement spanned ${dependent( "retirement_years")} years.`,
  ...
  `${toggle("Video games", "toggle_video_games")}: <span class='toggle_content'>You started to play video games when you were ${clickable( "video_game_start_age")}. When you were young, your parents allowed you to play 1 hour per day. But you managed to play ${clickable( "actual_play_time")} hours per day.<br><br></span>
...
<span class='toggle_content'>Strangely enough, you did not play video games much while you were in college. However, after you graduated from college because you needed to entertain yourself while not working, you played video games for about ${clickable( "play_time_after_working")} hour on a daily basis. That's a total of ${dependent( "total_video_game_hours")} hours in your lifetime.</span>`,
  ...
  `${toggle("Dating apps", "toggle_dating_apps")}: <span class='toggle_content'>When you were between the age of ${clickable( "dating_apps_start_age")} and ${clickable( "dating_apps_end_age")}, because you were lonely, you spent on average ${clickable( "dating_apps_daily_time")} hours per day on dating apps.</span>`,

  // because you were single, and due to the digital culture of the time, and a lack of opportunity to meet new people in real life ----->  because you were lonely ~ 2025.8.30

  `${toggle("A Long-lasting relationship", "toggle_love")}:<span class='toggle_content'> You found the love of your life when you were ${clickable( "fall_in_love_age")}, and since then you have spent ${clickable( "long_last_relationship_hours_per_day")} hours with the person daily. Major activities include: eating, sleeping, talking, making plans, helping each other out, etc.</span>`,

  // This did not happen ~ 2025.8.30
...
`${toggle("Coolphones", "toggle_coolphone")}: <span class='toggle_content'>Coolphone was the phone to have during your lifetime. You switched to the newest coolphone every ${clickable( "old_coolphone_last_years")} years. Each coolphone costs ${clickable( "coolphone_cost")} dollars. You got your first coolphone when you were ${clickable( "first_coolphone_age")}. You had ${dependent("total_number_of_coolphones")} coolphones in total. And they cost ${dependent( "total_coolphone_cost")} dollars.</span>`,
...
<span class='toggle_content'>People say that starting a company is like riding a roller coaster. You quit your job at the age of ${clickable("age_quit_job")} and became a full-time entrepreneur. Since founding your company, you have worked ${clickable("startup_hours_per_day")} hours a day, ${clickable("startup_work_days_per_week")} days a week.<br><br></span>
...
<span class='toggle_content'>The startup was your life's work. You retired from running the business at the age of ${dependent("startup_retire_age")} and let other people run the business. Since you have an ownership in the company, you still enjoy the same income after you retire.</span>`
];
