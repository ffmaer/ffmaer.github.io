    ...
    if(dict[item]()!=0){
      ...
      while (width > 0) {
        ...
        if (dict[item]() < 0 && item == "Money left") {
          itemdiv.style["background-color"] = "Indigo";
          if (Math.floor(width / longest_div_width) == 1) {
            itemdiv.innerHTML = `<span>Deficit</span>`;
          } else {
          ...
          itemdiv.style["background-color"] = item_count % 2 == 0 ? "MediumPurple" : "MediumPurple";
...
function winning(){
  if(isWinning() && !document.querySelector(".winning_message") && !is_winning_message_shown){
    ...
    let pattern = document.createElement("div");
    pattern.innerHTML = `
    <p speed='600' class='hidden_pattern'>The canvas of life—how will you choose to paint it?</p>
    
    <p speed='600' class='hidden_pattern'>There’s no need to dwell on the inevitable end.</p>

    <p speed='600' class='hidden_pattern'>An average life of 81 years spans roughly 700,000 hours, yet the truly brightest moments may last less than one.</p>
    
    <p speed='600' class='hidden_pattern'>Return often to those luminous moments—their glow can warm and illuminate your entire life.</p>
